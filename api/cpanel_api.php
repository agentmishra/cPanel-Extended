...
class cPanelApi {
	...
	public function accountsummary() {
		...
		return $this->sendRequest('json-api/accountsummary', array(
			'user' => $this->username
		));
	}
	/**
	...
	public function checkConnection() {
		if(empty($this->hostname) or empty($this->serverusername) or empty($this->key))
			return false;
	...
	protected function _curlExecute() {
		...
		if(!empty($this->serverpassword)) {
			curl_setopt($curl, CURLOPT_HTTPHEADER, array(
				"Authorization: Basic " . base64_encode($this->serverusername . ":" . $this->serverpassword) . "\n\r"
			));
		} else {
			curl_setopt($curl, CURLOPT_HTTPHEADER, array(
				"Authorization: WHM $this->serverusername:" . preg_replace("'(\r|\n)'", "", $this->key)
			));
		}
